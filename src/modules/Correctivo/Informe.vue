...
import { getInforme } from '@/services/__mocks__/correctivo/informe';
...
// route.params.idMantenimiento
const rporte = ref([] as any);

const btnPdfWeb = async () => {
    try {
        await presentLoading('Generando Informe...');
        ...
        await showToast('Error al obtener el reporte', 'danger', closeCircle)
    } finally {
...
<template>
    <HeaderBack titulo="Informe Mantenimiento Correctivo" color="orange" :id="id" />
        ...
        <div class="ion-padding" v-for="rpt in rporte">
            ...
            <fieldset v-if="rpt.datosGrua">
                ...
                <div v-if="rpt.datosGrua">
                    ...
                    <ion-text color="dark-a" class="ion-text-center">
                        <h4> <span class="negrita"> TAG / IDENTIFICACION DE GRUA:</span> <span>{{ rpt.datosGrua.Tag
                        }}</span>
                        </h4>
                ...
                <table>
                    ...
                    <tbody v-for="(titulos, index) in rpt.titulos">
                        ...
                        <tr v-for="(items, indexItem) in titulos.items">
                            <td class="wide-column tdItems"> <b>{{ index + 1 }}.{{ indexItem + 1 }}</b> {{ items.item }}
                            </td>
            ...
            <fieldset>
                <legend class="colorMantemiento">ACTIVIDADES REALIZADAS</legend>
                <div v-for="(anexo, index) in rpt.actividades">
                    <ion-text color="dark-a">
                        <h5>{{ index + 1 }}. {{ anexo.descripcion }}</h5>
                    </ion-text>
                    <ion-row>
            ...
            <fieldset>
                <legend class="colorMantemiento"> CONCLUSIONES Y RECOMENDACIONES </legend>
                <ion-text color="dark-a">
                    <b>{{ rpt.datos.Nombres }}</b> ha realizado el servicio de mantenimiento
                    <b>{{ rpt.datos.Frecuencia }}</b> del monoriel marca <b>{{ rpt.datos.Marcas }}</b> de <b>{{
                        rpt.datos.Capacidad }}</b> de carga, quedando en
                    condiciones <b>{{ rpt.estado }}</b>
                </ion-text>
                <ion-text color="dark-a" v-for="estado in rpt.recomendaciones">
